...
export function ClientWorkspaceAnalyticsBoard({
  ...
  const ringTotal = Math.max(totalBookings, 1);
  const segments = [
  ...
  const ringSegments = segments.map((segment) => {
    const dash = (segment.value / ringTotal) * circumference;
    const result = {
      dash,
  ...
  return (
    <div className="mt-6 rounded-[30px] border border-[var(--line)] bg-[rgba(255,255,255,0.96)] p-5 shadow-[0_18px_44px_rgba(15,23,42,0.08)] sm:p-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          <p className="text-[10px] uppercase tracking-[0.18em] text-[var(--brand-primary)]">Booking analytics</p>
          <h3 className="mt-2 font-display text-2xl tracking-[-0.04em] text-[var(--text-primary)] sm:text-3xl">See booking pressure, protected flow, and spend movement before you dive into the rail</h3>
          <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">
            This board keeps discovery, payment readiness, live delivery, and disputes visually separate so the workspace reads like a client control desk.
          </p>
        </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
        <div className="rounded-[26px] border border-[var(--line)] bg-[rgba(248,250,252,0.94)] p-5">
          <div className="flex items-center justify-between gap-3">
            <p className="text-xs uppercase tracking-[0.16em] text-[var(--text-tertiary)]">Booking mix</p>
          ...
          <div className="mt-5 space-y-3">
            {[
              { label: 'Active delivery', value: activeDeliveries, color: 'var(--accent-teal)' },
              { label: 'Pending collection', value: pendingCollections, color: 'var(--accent-amber)' },
              { label: 'Disputes', value: disputedBookings, color: 'var(--accent-coral)' },
              { label: 'Open planning', value: openBookings, color: 'var(--accent-cyan)' },
            ].map((item) => (
        ...
        <div className="space-y-6">
          <div className="grid gap-3 md:grid-cols-3">
            {[
              ['Latest value', formatCompactMoney(safeTrendPoints[safeTrendPoints.length - 1]?.value ?? 0)],
              ['Peak value', formatCompactMoney(Math.max(...safeTrendPoints.map((point) => point.value), 0))],
          ...
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
            <div className="rounded-[26px] border border-[var(--line)] bg-[rgba(248,250,252,0.94)] p-5">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.16em] text-[var(--text-tertiary)]">Protected value trend</p>
                  <p className="mt-2 text-sm text-[var(--text-secondary)]">Recent booking amounts already flowing through the client workspace.</p>
                </div>
              ...
              <div className="mt-5">
                <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="h-44 w-full">
                  ...
                  <path d={trendArea} fill="url(#clientTrendFill)" />
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-2">
              <div className="rounded-[22px] border border-[rgba(56,189,248,0.16)] bg-[rgba(240,249,255,0.94)] p-4 transition duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-0.5 hover:shadow-[0_16px_34px_rgba(15,23,42,0.07)]">
                <p className="text-[10px] uppercase tracking-[0.16em] text-[var(--text-tertiary)]">Lane results</p>
                <p className="mt-3 font-display text-2xl text-[var(--text-primary)]">{categoryResultsCount}</p>
                <p className="mt-2 text-sm text-[var(--text-secondary)]">Current visible lanes</p>
              </div>
              <div className="rounded-[22px] border border-[rgba(139,92,246,0.16)] bg-[rgba(245,243,255,0.94)] p-4 transition duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-0.5 hover:shadow-[0_16px_34px_rgba(15,23,42,0.07)]">
                <p className="text-[10px] uppercase tracking-[0.16em] text-[var(--text-tertiary)]">Tracked bookings</p>
                <p className="mt-3 font-display text-2xl text-[var(--text-primary)]">{trackedBookings}</p>
                <p className="mt-2 text-sm text-[var(--text-secondary)]">Protected workflow in view</p>
              </div>
              <div className="rounded-[22px] border border-[rgba(20,184,166,0.16)] bg-[rgba(240,253,250,0.94)] p-4 transition duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-0.5 hover:shadow-[0_16px_34px_rgba(15,23,42,0.07)]">
                <p className="text-[10px] uppercase tracking-[0.16em] text-[var(--text-tertiary)]">Active delivery</p>
                <p className="mt-3 font-display text-2xl text-[var(--text-primary)]">{activeDeliveries}</p>
                <p className="mt-2 text-sm text-[var(--text-secondary)]">Work currently moving</p>
              </div>
              <div className="rounded-[22px] border border-[rgba(249,115,22,0.16)] bg-[rgba(255,247,237,0.94)] p-4 transition duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-0.5 hover:shadow-[0_16px_34px_rgba(15,23,42,0.07)]">
                <p className="text-[10px] uppercase tracking-[0.16em] text-[var(--text-tertiary)]">Protected value</p>
                <p className="mt-3 font-display text-2xl text-[var(--text-primary)]">{formatCompactMoney(protectedValue)}</p>
                <p className="mt-2 text-sm text-[var(--text-secondary)]">Recent escrow-linked amount</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            {[
              { label: 'Lane coverage', value: catalogCoverage, tone: 'var(--accent-cyan)' },
              { label: 'Protected flow', value: protectedFlow, tone: 'var(--accent-teal)' },
              { label: 'Delivery pressure', value: deliveryPressure, tone: 'var(--accent-amber)' },
            ].map((item) => (
